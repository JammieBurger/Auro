...
LINEAR_VELOCITY = 0.1  # Slightly faster movement
ANGULAR_VELOCITY = 0.4  # Faster turning
COLLECTION_DISTANCE = 0.35  # More forgiving collection distance
ZONE_APPROACH_DISTANCE = 0.5
OBSTACLE_THRESHOLD = 0.2  # Less sensitive to avoid false wall detection
APPROACH_ATTEMPTS_MAX = 2  # Fewer attempts before giving up
...
class ItemCollectorRobot(Node):
    ...
    def approach_item(self):
        ...
        if estimated_distance <= COLLECTION_DISTANCE * 1.2:  # More conservative threshold
        ...
        # More aggressive steering toward item
        #twist.angular.z = -current_item.x * 0.005
        self.cmd_vel_pub.publish(twist)
    ...
    def fine_approach_item(self):
        ...
        if estimated_distance <= COLLECTION_DISTANCE * 0.7:
            self.state = RobotState.COLLECTING_ITEM
            return
        
        # Fine movement towards item
        twist = Twist()
        twist.linear.x = LINEAR_VELOCITY * 0.5  # Slower approach
        
        # Fine angular adjustment
        #if abs(current_item.x) > 15:  # Only adjust if significantly off-center
            #twist.angular.z = -current_item.x * 0.003
        
        self.cmd_vel_pub.publish(twist)
    
    def estimate_distance_to_item(self, item):
        """CONSERVATIVE distance estimation - robot must get much closer"""
        # Much more conservative estimates - robot needs to be very close
        if item.diameter > 100:
            return 0.15
        elif item.diameter > 80:
            return 0.25
        elif item.diameter > 60:
            return 0.4
        elif item.diameter > 40:
            return 0.6
        elif item.diameter > 25:
            return 0.9
        else:
            return 1.5
    ...
    def approach_zone(self):
        ...
        #twist.angular.z = -current_zone.x * 0.004  # Proportional control
        self.cmd_vel_pub.publish(twist)
