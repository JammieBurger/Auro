...
class ItemCollectorRobot(Node):
    def __init__(self):
        ...
        self.get_logger().info('Item Collector Robot initialized')
    ...
    def scan_callback(self, msg):
        ...
        front_angles = list(range(350, 360)) + list(range(0, 10))
        front_distances = [msg.ranges[i] for i in front_angles if i < len(msg.ranges)]
        
        self.obstacle_detected = any(d < OBSTACLE_THRESHOLD for d in front_distances if not math.isinf(d))
    ...
    def item_holders_callback(self, msg):
        ...
        for holder in self.item_holders:
            if holder.robot_id == 'robot1' and holder.holding_item:
    ...
    def control_loop(self):
        ...
        # Handle obstacle avoidance with priority
        if self.obstacle_detected and self.state not in [RobotState.AVOIDING_OBSTACLE, RobotState.DEPOSITING_ITEM]:
            self.previous_state = self.state
            self.state = RobotState.AVOIDING_OBSTACLE
            self.avoidance_start_yaw = self.yaw
            self.avoidance_turn_angle = random.uniform(90, 135) * random.choice([-1, 1])
            self.get_logger().info(f'Obstacle detected! Switching to AVOIDING_OBSTACLE from {self.previous_state}')
    ...
    def approach_item(self):
        ...
        # Move towards item
        twist.linear.x = min(LINEAR_VELOCITY, estimated_distance * 0.5)
        twist.angular.z = -current_item.x * 0.005  # Proportional control for alignment
        self.cmd_vel_pub.publish(twist)
    ...
    def avoid_obstacle(self):
        ...
        angle_turned = angles.normalize_angle(self.yaw - self.avoidance_start_yaw)
