...
class ItemCollectorRobot(Node):
    def __init__(self):
        ...
        # Zone waiting mechanism - NEW
        self.zone_wait_counter = 0
        self.zone_wait_duration = 20  # Wait for 2 seconds (20 * 0.1s control loop)
        self.is_waiting_over_zone = False
    ...
    def reset_zone_waiting(self):
        """Reset zone waiting state when changing states"""
        self.is_waiting_over_zone = False
        self.zone_wait_counter = 0
    ...
    def search_for_items(self):
        ...
        # Reset zone waiting when starting to search for items
        self.reset_zone_waiting()
    ...
    def estimate_distance_to_item(self, item):
        ...
        if item.diameter > 340:
            return 0.15
        elif item.diameter > 310:
            return 0.30
        elif item.diameter > 290:
            return 0.40
            ...
            return 0.75
        elif item.diameter > 230:
            return 1.0
        elif item.diameter > 180:
            return 1.3
    ...
    def approach_zone(self):
        """Navigate towards the target zone using camera feedback with wait timer"""
        ...
        # Enhanced zone waiting logic with timer
        if current_zone.size > 0.99:
            twist = Twist()
            self.cmd_vel_pub.publish(twist)
            
            if not self.is_waiting_over_zone:
                # Just arrived over the zone, start waiting
                self.is_waiting_over_zone = True
                self.zone_wait_counter = 0
                
                # Adjust wait time based on item value (higher value = longer wait for accuracy)
                if self.held_item_colour == 'BLUE':
                    self.zone_wait_duration = 30  # 3 seconds for high-value blue items
                elif self.held_item_colour == 'GREEN':
                    self.zone_wait_duration = 20  # 2 seconds for medium-value green items
                else:  # RED items
                    self.zone_wait_duration = 15  # 1.5 seconds for low-value red items
                
                wait_time = self.zone_wait_duration * 0.1
                self.get_logger().info(f'Over zone with {self.held_item_colour} item, waiting {wait_time:.1f}s before deposit...')
            
            # Increment wait counter
            self.zone_wait_counter += 1
            
            # Fine positioning while waiting (micro-adjustments)
            if abs(current_zone.x) > 2:  # Very fine centering while waiting
                twist.angular.z = current_zone.x * 0.001  # Very small adjustments
                twist.angular.z = max(-0.1, min(0.1, twist.angular.z))
                self.cmd_vel_pub.publish(twist)
            
            # Check if we've waited long enough
            if self.zone_wait_counter >= self.zone_wait_duration:
                self.state = RobotState.DEPOSITING_ITEM
                self.is_waiting_over_zone = False
                self.zone_wait_counter = 0
                self.get_logger().info(f'Wait complete, attempting deposit')
            else:
                # Still waiting, show progress every 10 iterations (1 second)
                if self.zone_wait_counter % 10 == 0:
                    remaining_time = (self.zone_wait_duration - self.zone_wait_counter) * 0.1
                    self.get_logger().info(f'Centering and waiting... {remaining_time:.1f}s remaining')
            
            return
        else:
            # Not over zone anymore, reset waiting state
            if self.is_waiting_over_zone:
                self.get_logger().warn('Lost zone contact while waiting, resuming approach')
                self.is_waiting_over_zone = False
                self.zone_wait_counter = 0
    ...
    def offload_callback(self, future):
        ...
        try:
            ...
            if response.success:
                ...
                # Reset zone waiting state after successful deposit
                self.reset_zone_waiting()
                self.state = RobotState.SEARCHING
            else:
